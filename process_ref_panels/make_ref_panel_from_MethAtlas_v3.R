## What's changed in V3 from V2:
##  1. Removed the step of iteratively adding probes to distinguish most similar cell types
...
colSums(full_diff_max.df[, -1] > 0 & full_diff_mean.df[, -1] > 0.2) |> sort(decreasing = TRUE)
#  0%   25%   50%   75%  100% 
...
saveRDS(ref_cts.se, here(write_dir, 'summarizedexperiment_cell_type_specific_atlas_v3.rds'))
...
saveRDS(ref_anc.se, here(write_dir, 'summarizedexperiment_all_tissue_um_atlas_v3.rds'))
...
saveRDS(ref_cts_lifted.se, here(write_dir, 'summarizedexperiment_cell_type_specific_atlas_hg38_v3.rds'))
...
saveRDS(ref_anc_lifted.se, here(write_dir, 'summarizedexperiment_all_tissue_um_atlas_hg38_v3.rds'))
