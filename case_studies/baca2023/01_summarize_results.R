...
write_dir <- here('data', 'interim', 'case_studies', 'baca2023', '01_summarize_results')
...
for (i in seq_len(nrow(md_samples))) {
  ...
  sample_pi.df <- fread(here(read_dir, 'NM_output', paste0('fitted_pi_', sample,'.csv.gz'))) |>
...
for (i in seq_len(nrow(md_samples))) {
  ...
  sample_w_mu.df <- fread(here(read_dir, 'NM_output', paste0('fitted_w_mu_', sample,'.csv.gz'))) |>
...
for (i in seq_len(nrow(md_samples))) {
  ...
  sample_w_sigma.df <- fread(here(read_dir, 'NM_output', paste0('fitted_w_sigma_', sample,'.csv.gz'))) |>
    select(mean, `2.5%`, `25%`, `50%`, `75%`, `97.5%`) |>
