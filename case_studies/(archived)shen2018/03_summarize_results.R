...
read_dir <-  here('data', 'interim', 'case_studies', 'shen2018', '02_apply_deconv')
write_dir <- here('data', 'interim', 'case_studies', 'shen2018', '03_summarize_results')
if (!file.exists(write_dir)) dir.create(write_dir, recursive = T)
plot_dir <- here('plots', 'case_studies', 'shen2018', '03_summarize_results')
...
BINWIDTH <- 1
samples_cts.se <- readRDS(here(read_dir, '..', '01_get_read_counts_refv2', paste0('summarizedexperiment_samples_cts_regions_bw', BINWIDTH, '.rds')))
samples_anc.se <- readRDS(here(read_dir, '..', '01_get_read_counts_refv2', paste0('summarizedexperiment_samples_anc_regions_bw', BINWIDTH, '.rds')))

md_samples <- colData(samples_cts.se) |>
  as.data.frame() |>
  mutate(cts_total_reads = colSums(assays(samples_cts.se)[[1]])) |>
  mutate(group = relevel(factor(group), ref = 'Control'))


# --- check coverage of the samples ----
md_samples |>
  ggplot(aes(cts_total_reads, color = group)) +
  geom_density() +
  theme_classic()
...
ggsave(here(plot_dir, paste0('boxplot_deconvProp_vs_patientGroup_', model_name, '.png')), width = 15, height = 12)
...
filtered_samples_pi.df %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylab('Estimated cell type proportions')
ggsave(here(plot_dir, paste0('boxplot_deconvProp_vs_patientGroup_', model_name, '_filteredLowCoverage.png')), width = 15, height = 12)
...
filtered_samples_pi.df %>%
  ...
  # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab('Estimated cell type proportions')
ggsave(here(plot_dir, paste0('boxplot_cellType_vs_deconvProp_', model_name, '_filteredLowCoverage.png')), width = 12, height = 15)


#### barplot per sample (does NOT look very informative)
...
#   xlab('sample') +
#   ylab('Estimated read counts of tissue of origin') +
#   scale_fill_manual(values = getPalette(color_count)) +
#   guides(fill = guide_legend(ncol = 1))
# 
# all_samples_pi.df |>
#   mutate(estimated_read_counts = mean * cts_total_reads) |>
#   arrange(desc(estimated_read_counts)) |>
#   mutate(sample = fct_reorder(sample, estimated_read_counts, .fun = max, .desc = TRUE)) |>
#   ggplot(aes(fill = cell_type, y = estimated_read_counts, x = sample)) +
#   geom_bar(position = "stack", stat = "identity", width = 0.5, color = 'white') +
#   facet_grid(~ group, scales = 'free_x', space = 'free') +
#   theme_classic() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
# 
#   total_reads <- countBam(BamFile(bam_file))$records
# 
#   sample_w_mu.df <- fread(here(read_dir, paste0('fitted_w_mu_', model_name, '_', sample,'.csv'))) |>
#     select(mean, `2.5%`, `25%`, `50%`, `75%`, `97.5%`) |>
#     mutate(sample = sample, group = group, million_reads = total_reads/1e6, parameter = paste0('w_mu_', c(1:(n()-1), 0))) |>
...
#     select(mean, `2.5%`, `25%`, `50%`, `75%`, `97.5%`) |>
#     mutate(sample = sample, group = group, million_reads = total_reads/1e6, parameter = paste0('w_sigma_', c(1:(n()-1), 0))) |>
#     relocate(sample, group, million_reads, parameter)
# 
